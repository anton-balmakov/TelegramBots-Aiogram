...
async def registration(message: Message):
    ...
    cursor.execute("SELECT * FROM users_test WHERE telegram_id = ?", (telegram_id,))
    user = cursor.fetchone()
    if user:
        await message.answer("Вы уже зарегистрированы в боте")
    else:
        cursor.execute('''INSERT INTO users_test (telegram_id, name) VALUES (?, ?)''', (telegram_id, name))
        conn.commit()
...
async def finances(message: Message, state: FSMContext):
    ...
    telegram_id = message.from_user.id
    cursor.execute('''
    UPDATE users_test SET category1 = ?, expenses1 = ?, category2 = ?, expenses2 = ?, category3 = ?, expenses3 = ? WHERE telegram_id = ?''',
